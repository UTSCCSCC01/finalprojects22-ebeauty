...
const SearchBox = ({ providers, service }) => {
  ...
  if (searchKeyword) {
    ...
    const similarityNameArray = providers
      .map((provider) => {
        return {
          provider: provider,
          similarity: similarity(provider.name, searchKeyword),
        };
      })
      .sort((a, b) => {
        return b.similarity - a.similarity;
    ...
    if (
      similarityServiceArray[0].similarity > similarityNameArray[0].similarity
    ) {
      similar = similarityServiceArray[0].service.value;
      similarityScore = similarityServiceArray[0].similarity;
    } else {
      similar = similarityNameArray[0].provider.name;
      similarityScore = similarityNameArray[0].similarity;
    }
